...
## D-020：例句创建使用独立的最小持久安全账本

**日期：** 2026-09-13
**状态：** 有效；Owner 在 Issue #161 评论 5647929027 选择 B

真实使用已证明：CREATE 成功后，列表 GET 可能暂时漏项，进而诱导重复创建并超过五条上限。安全校验后的 CREATE phrase 响应可证明创建成功，仍须鉴权 GET 回读；单纯列表漏项不再将已证明创建降为失败。

使用独立于 D-017 History 的小型、版本化 Foundation Codable 文件，存于 Application Support，原子替换并尽力排除备份；不使用 UserDefaults、数据库、Keychain、缓存或云同步。仅保存按账号、词条、例句身份和 provider 资源划分的完整 SHA-256 摘要与时间。额外的英文及忽略来源的身份摘要用于维持相同英文冲突与可选来源规则；不保存原始指纹、ID、词头、句子、翻译、来源、标签或请求/响应正文。

待同步条目跨重启抑制相同 CREATE，并与可见有效资源及批次预留数量一起计入五条上限。鉴权列表出现对应资源 ID 后不重复计数，并尽力清退待同步条目；仅列表缺失不得清退。清空 History 不清空账本，账号更换/移除也不盲删账本，不同账号摘要的证据互不参与计算。
