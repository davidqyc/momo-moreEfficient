...
- **共享**：在系统分享面板里把文本交给小黑鸟伴侣，正常打开 App 后进入“抓词”状态；一次性把小黑鸟伴侣加入常用分享目标后，直接从阅读 App 里选中文字发起，不需要为每个来源单独配置。

更快的预配置备选入口：

- **快捷指令（iOS 26+）**：把选中的文本交给小黑鸟伴侣的抓词 Shortcut 动作，App 会自动前台打开到同一个“抓词”状态；配置一次后每次只需一次运行。
...
| 阅读时抓词 | **即将上线** | 源码已完成，真机体验已验证；共享为推荐日常入口，快捷指令为更快的预配置备选；下一版发布前还会做一次统一 copy/架构复查 |
