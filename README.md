...
几个方向的状态刻意分开写，避免把“源码里有”误认为“用户现在已经能用”：

| 能力 | 当前状态 |
| --- | --- |
| iPhone 释义 / 例句安全录入 | **已在 TestFlight build 1.0 (3) 提供** |
| App Intent / Shortcut / Action Button 抓词 | **源码已完成；尚未进入当前 TestFlight** |
| iOS Share Extension 抓词 | **源码已完成；尚未进入当前 TestFlight；待 App Group / 真机验证** |
| 桌面浏览器抓词 Extension | **尚未实现；等待墨墨开放平台明确 OAuth 回调 / CORS / API 合同** |
| 墨墨内置词典 / 发音服务 | **本项目目前不声称提供** |
| 后台自动导入队列 | **未提供** |
