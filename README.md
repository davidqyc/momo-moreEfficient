...
**小黑鸟伴侣（momo-moreEfficient）**是一个独立、非官方、开源的 **墨墨背单词实用助手 / Maimemo companion**。目的很简单：把墨墨里几件高频但麻烦的事做得更顺手——先是安全录入释义和例句，接着是阅读时抓词，以及可复现的 **Maimemo × Codex** 学习工作流。
...
两条入口都停在**预览之前**：仅仅抓词，不会读取墨墨 Token、不会访问墨墨、不会自动预览，更不会自动写入。

接下来会做一次真机体验比较。项目的目标是选出**实际更省事的那条日常路径**，而不是为了技术对称强行让用户长期同时维护两套入口。

## 功能 3｜Maimemo × Codex：把墨墨学习数据变成可复现的学习材料

这个路线不依赖 iPhone App，也不是一个“大而全的 AI 平台”。每个 Recipe 都应该是一个小而明确、可以复现的工作流。
...
没上线的能力按阶段写，避免把“源码里有”误认为“用户现在已经能用”：

| 能力 | 当前阶段 |
| --- | --- |
| iPhone 释义 / 例句安全录入 | **已上线**：当前 TestFlight build `1.0 (3)` 已提供 |
| 快捷指令抓词 | **即将上线**：源码已完成；尚未进入当前 TestFlight，待真机体验比较 |
| 共享抓词 | **即将上线**：源码已完成；尚未进入当前 TestFlight，待 App Group / 真机验证 |
| 桌面浏览器抓词 Extension | **调研中**：尚未实现；等待墨墨开放平台明确 OAuth 回调 / CORS / API 合同 |
| 墨墨内置词典 / 发音服务 | **暂未提供** |
| 后台自动导入队列 | **暂无计划** |
