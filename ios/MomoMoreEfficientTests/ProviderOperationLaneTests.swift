import Foundation
import XCTest
@testable import MomoMoreEfficient

/// The application-level provider operation lane, and the narrow read seam
/// batch Query is given (adjudication findings 1–5).
///
/// The shared `RequestWindowScheduler` is a *rate ledger*: it paces requests
/// against the documented windows but does not by itself prove that only one
/// request is in flight. These tests therefore observe the **actual fake
/// transport**, not scheduler reservations.
@MainActor
final class ProviderOperationLaneTests: XCTestCase {

    // MARK: - Actual in-flight observation

    func testAQueryRunNeverHasMoreThanOneRequestInFlight() async throws {
        let store = QuerySessionStore()
        store.updateInput("alpha\nbeta\ngamma")

        let transport = ConcurrencyObservingHTTPTransport([
            resolvedQueryResponse(["alpha", "beta", "gamma"]),
            interpretationsResponse([interpretation("I1", "n. a")]),
            phrasesResponse([]),
            notesResponse([]),
            interpretationsResponse([]),
            phrasesResponse([]),
            notesResponse([]),
            interpretationsResponse([]),
            phrasesResponse([]),
            notesResponse([]),
        ])
        store.start(lease: try queryLease(transport))
        await store.awaitRunCompletion()

        XCTAssertEqual(store.phase, .completed)
        XCTAssertEqual(transport.readCount, 10)
        // The real proof: never two overlapping requests, at any point.
        XCTAssertEqual(transport.maximumConcurrentRequests, 1)
        XCTAssertEqual(transport.postCount, 0)
    }

    // MARK: - The lane excludes incompatible operations

    func testQueryCannotTakeTheLaneWhileAPreviewIsReading() async throws {
        // Parks on the Preview's very first (resolver) request.
        let stepped = SteppedHTTPTransport([])
        let model = makeConnectedModel(transport: stepped)
        model.sourceText = "## merchandise\nn. 商品"

        XCTAssertNil(model.activeProviderOperation)
        let preview = Task { await model.previewCurrentInput() }
        await stepped.waitUntilParked()

        XCTAssertEqual(model.activeProviderOperation, .preview)
        XCTAssertTrue(model.isProviderLaneBusy)
        // Query simply does not start rather than racing the Preview.
        XCTAssertNil(model.beginQueryRead())

        // Let the Preview end (a transport failure aborts its read plan).
        await stepped.release(.failure(.transport))
        await preview.value

        XCTAssertNil(model.activeProviderOperation)
        XCTAssertFalse(model.isProviderLaneBusy)
        // …and now Query may take the lane.
        let lease = try XCTUnwrap(model.beginQueryRead())
        lease.finish()
    }

    func testQueryCannotTakeTheLaneWhileCredentialValidationIsInFlight() async throws {
        let gated = GatedHTTPTransport(
            vocabularyResponse("INVALID_VALIDATION_VOC", "apple")
        )
        let model = CompanionViewModel(phraseSafetyJournal: makeTestPhraseJournal(),
            tokenStore: FakeTokenStore(),
            historyStore: InMemoryHistoryStore(),
            credentialValidationTransportFactory: { gated },
            sleeperFactory: { RecordingSleeper() },
            preferenceDefaults: isolatedPreferenceDefaults()
        )
        let connection = Task { await model.connect(token: fakeToken) }
        await gated.waitUntilRequested()

        XCTAssertEqual(model.activeProviderOperation, .credentialValidation)
        XCTAssertNil(model.beginQueryRead())

        await gated.resume()
        _ = await connection.value
        XCTAssertNil(model.activeProviderOperation)
    }

    func testPreviewAndWritesCannotStartWhileQueryOwnsTheLane() async throws {
        let model = makeConnectedModel(transport: FakeHTTPTransport([]))
        model.sourceText = "## merchandise\nn. 商品"

        let lease = try XCTUnwrap(model.beginQueryRead())
        XCTAssertEqual(model.activeProviderOperation, .query)

        // A Preview started now takes no lane and dispatches nothing.
        await model.previewCurrentInput()
        XCTAssertFalse(model.isPreviewing)
        XCTAssertNil(model.preview)

        // An authorized write is refused while Query still holds the lane.
        XCTAssertNil(model.executeConfirmedWholePlan())
        XCTAssertNil(model.executeConfirmed(.create))
        XCTAssertNil(model.executeConfirmedPhrase())

        lease.finish()
        XCTAssertNil(model.activeProviderOperation)
    }

    func testTheLeaseIsIdempotentAndAlwaysReleasesTheLane() throws {
        let model = makeConnectedModel(transport: FakeHTTPTransport([]))
        let lease = try XCTUnwrap(model.beginQueryRead())
        XCTAssertEqual(model.activeProviderOperation, .query)

        lease.finish()
        lease.finish()
        XCTAssertNil(model.activeProviderOperation)

        // The lane is reusable afterwards.
        let second = try XCTUnwrap(model.beginQueryRead())
        XCTAssertEqual(model.activeProviderOperation, .query)
        second.finish()
    }

    func testQueryIsRefusedWhenThereIsNoConnection() {
        let model = CompanionViewModel(phraseSafetyJournal: makeTestPhraseJournal(),
            tokenStore: FakeTokenStore(),
            historyStore: InMemoryHistoryStore(),
            credentialValidationTransportFactory: { FakeHTTPTransport([]) },
            sleeperFactory: { RecordingSleeper() },
            preferenceDefaults: isolatedPreferenceDefaults()
        )
        XCTAssertFalse(model.isConnected)
        XCTAssertNil(model.beginQueryRead())
        XCTAssertNil(model.activeProviderOperation)
    }

    // MARK: - Identity semantics (findings 4 and 5)

    func testASuccessfulConnectAndRemovalAdvanceTheAccountAuthority() async {
        let store = FakeTokenStore()
        let transport = FakeHTTPTransport([
            vocabularyResponse("INVALID_VALIDATION_VOC", "apple"),
            vocabularyResponse("INVALID_VALIDATION_VOC", "apple"),
        ])
        let model = CompanionViewModel(phraseSafetyJournal: makeTestPhraseJournal(),
            tokenStore: store,
            historyStore: InMemoryHistoryStore(),
            credentialValidationTransportFactory: { transport },
            sleeperFactory: { RecordingSleeper() },
            preferenceDefaults: isolatedPreferenceDefaults()
        )
        XCTAssertEqual(model.accountIdentity, .disconnected)

        let connected = await model.connect(token: fakeToken)
        XCTAssertTrue(connected)
        let afterConnect = model.accountIdentity
        XCTAssertNotNil(afterConnect.fingerprint)
        XCTAssertEqual(afterConnect.authorityGeneration, 1)

        model.removeToken()
        XCTAssertNil(model.accountIdentity.fingerprint)
        XCTAssertEqual(model.accountIdentity.authorityGeneration, 2)
    }

    func testAFailedCandidateChangesNeitherIdentityDimension() async {
        let transport = FakeHTTPTransport([
            vocabularyResponse("INVALID_VALIDATION_VOC", "apple"),
            jsonResponse(["error": "unauthorized"], status: 401),
        ])
        let model = CompanionViewModel(phraseSafetyJournal: makeTestPhraseJournal(),
            tokenStore: FakeTokenStore(),
            historyStore: InMemoryHistoryStore(),
            credentialValidationTransportFactory: { transport },
            sleeperFactory: { RecordingSleeper() },
            preferenceDefaults: isolatedPreferenceDefaults()
        )
        let connected = await model.connect(token: fakeToken)
        XCTAssertTrue(connected)
        let before = model.accountIdentity

        let replaced = await model.connect(token: "CANDIDATE_TOKEN_NOT_VALID")
        XCTAssertFalse(replaced)
        // Nothing was mutated, so account-derived truth must not be cleared.
        XCTAssertEqual(model.accountIdentity, before)
    }

    func testAFailedRemovalLeavesTheIdentityAndConnectionIntact() async {
        let store = FakeTokenStore()
        let model = CompanionViewModel(phraseSafetyJournal: makeTestPhraseJournal(),
            tokenStore: store,
            historyStore: InMemoryHistoryStore(),
            credentialValidationTransportFactory: { successfulCredentialValidationTransport() },
            sleeperFactory: { RecordingSleeper() },
            preferenceDefaults: isolatedPreferenceDefaults()
        )
        let connected = await model.connect(token: fakeToken)
        XCTAssertTrue(connected)
        let before = model.accountIdentity

        store.failDelete = true
        model.removeToken()

        // Truthful: nothing was deleted, so the app still says 已连接.
        XCTAssertTrue(model.isConnected)
        XCTAssertEqual(model.accountIdentity, before)
        XCTAssertNotNil(model.errorMessage)
    }

    func testAQueryAuthenticationRejectionDisconnectsWithoutChangingIdentity() async {
        let model = CompanionViewModel(phraseSafetyJournal: makeTestPhraseJournal(),
            tokenStore: FakeTokenStore(),
            historyStore: InMemoryHistoryStore(),
            credentialValidationTransportFactory: { successfulCredentialValidationTransport() },
            sleeperFactory: { RecordingSleeper() },
            preferenceDefaults: isolatedPreferenceDefaults()
        )
        let connected = await model.connect(token: fakeToken)
        XCTAssertTrue(connected)
        let before = model.accountIdentity

        model.handleQueryAuthenticationRejection()

        // The session drops and the Owner is routed to Settings …
        XCTAssertFalse(model.isConnected)
        // … but this is not an account change, so a Query result built under
        // this identity stays truthful.
        XCTAssertEqual(model.accountIdentity, before)
    }

    func testABackgroundSuspensionIsNotAnAccountChange() async {
        let transport = FakeHTTPTransport([
            vocabularyResponse("INVALID_VALIDATION_VOC", "apple"),
            vocabularyResponse("INVALID_VALIDATION_VOC", "apple"),
        ])
        let model = CompanionViewModel(phraseSafetyJournal: makeTestPhraseJournal(),
            tokenStore: FakeTokenStore(),
            historyStore: InMemoryHistoryStore(),
            credentialValidationTransportFactory: { transport },
            sleeperFactory: { RecordingSleeper() },
            preferenceDefaults: isolatedPreferenceDefaults()
        )
        let connected = await model.connect(token: fakeToken)
        XCTAssertTrue(connected)
        let before = model.accountIdentity

        model.enterBackground()
        XCTAssertFalse(model.isConnected)
        XCTAssertEqual(model.accountIdentity, before, "suspension changed identity")

        // Restoring the same saved Token keeps the same identity, so nothing
        // account-derived is cleared.
        await model.enterForeground()
        XCTAssertTrue(model.isConnected)
        XCTAssertEqual(model.accountIdentity, before)
    }

    // MARK: - C-01: a Query 401 reaches the root session exactly once

    /// The production seam end to end: real transport → `QueryReadRunner` →
    /// `QuerySessionStore` → the root's existing session-failure path. Nothing
    /// here calls `handleQueryAuthenticationRejection()` by hand.
    func testAQuery401ReachesTheRootSessionThroughTheProductionPath() async throws {
        let transport = FakeHTTPTransport([
            resolvedQueryResponse(["alpha", "beta"]),
            interpretationsResponse([interpretation("I1", "n. a")]),
            phrasesResponse([]),
            notesResponse([]),
            jsonResponse(["error": "unauthorized"], status: 401),
        ])
        let model = makeConnectedModel(transport: transport)
        let store = QuerySessionStore()
        store.updateInput("alpha\nbeta")
        XCTAssertTrue(model.isConnected)
        let identityBefore = model.accountIdentity

        store.start(lease: try XCTUnwrap(model.beginQueryRead()))
        await store.awaitRunCompletion()

        // Query stops on the global authentication failure …
        XCTAssertEqual(store.phase, .stopped(.globalFailure(.authenticationRejected)))
        // … completed truth survives, and nothing unfinished becomes a fake 0 …
        XCTAssertEqual(store.rows[0].cell(.interpretation), .count(1))
        XCTAssertEqual(store.rows[0].cell(.phrase), .count(0))
        XCTAssertEqual(store.rows[1].cell(.interpretation), .unread)
        XCTAssertNil(store.rows[1].cell(.interpretation).knownCount)
        // … the root session actually heard about it …
        XCTAssertFalse(model.isConnected, "root still believes it is connected")
        // … but a rejection is not an account change …
        XCTAssertEqual(model.accountIdentity, identityBefore)
        // … the lane is released and no new lease is admitted …
        XCTAssertNil(model.activeProviderOperation)
        XCTAssertNil(model.beginQueryRead())
        // … and nothing retried: five reads, no mutation, ever.
        XCTAssertEqual(transport.readCount, 5)
        XCTAssertEqual(transport.postCount, 0)
    }

    func testAnOrdinaryReconnectAfterAQuery401AdmitsQueryAgain() async throws {
        let transport = FakeHTTPTransport([
            jsonResponse(["error": "unauthorized"], status: 401),
        ])
        let model = makeConnectedModel(transport: transport)
        let store = QuerySessionStore()
        store.updateInput("alpha")

        store.start(lease: try XCTUnwrap(model.beginQueryRead()))
        await store.awaitRunCompletion()
        XCTAssertFalse(model.isConnected)
        XCTAssertNil(model.beginQueryRead())

        // Only the Owner's own explicit reconnect re-admits Query. Nothing in
        // the failure path retried anything on its own.
        let reconnected = await model.connect(token: fakeToken)
        XCTAssertTrue(reconnected)
        let lease = try XCTUnwrap(model.beginQueryRead())
        lease.finish()
        XCTAssertEqual(transport.postCount, 0)
    }

    func testOneFailingRunReportsExactlyOneAuthenticationRejection() async throws {
        let rejections = CallCounter()
        let transport = FakeHTTPTransport([
            jsonResponse(["error": "unauthorized"], status: 401),
        ])
        let store = QuerySessionStore()
        store.updateInput("alpha")

        let lease = try queryLease(
            transport,
            onAuthenticationRejected: { rejections.record() }
        )
        store.start(lease: lease)
        await store.awaitRunCompletion()

        XCTAssertEqual(store.phase, .stopped(.globalFailure(.authenticationRejected)))
        XCTAssertEqual(rejections.count, 1)
        // The seam is once-per-lease, i.e. once per run, exactly like `finish()`.
        lease.reportAuthenticationRejection()
        XCTAssertEqual(rejections.count, 1)
    }

    /// Requirement 8: only a real 401 is a credential fact. A rate limit, a
    /// transport failure and an undecodable cell are ordinary read failures.
    func testNonAuthenticationFailuresNeverDisconnectTheRootSession() async throws {
        let rateLimited = FakeHTTPTransport([
            resolvedQueryResponse(["alpha"]),
            jsonResponse(["errors": ["too many requests"]], status: 429),
        ])
        let model = makeConnectedModel(transport: rateLimited)
        let store = QuerySessionStore()
        store.updateInput("alpha")

        store.start(lease: try XCTUnwrap(model.beginQueryRead()))
        await store.awaitRunCompletion()

        XCTAssertEqual(store.phase, .stopped(.globalFailure(.rateLimited)))
        XCTAssertTrue(model.isConnected, "a rate limit is not a credential rejection")
        XCTAssertNil(model.activeProviderOperation)

        // A per-cell decode failure is even more local: the run keeps going and
        // only that one cell is unavailable.
        let undecodable = FakeHTTPTransport([
            resolvedQueryResponse(["alpha"]),
            interpretationsResponse([interpretation("I1", "n. a")]),
            phrasesResponse([]),
            // A note record the closed decoder cannot accept.
            notesResponse([["id": "N1", "note": "x", "status": "ARCHIVED"]]),
        ])
        let second = makeConnectedModel(transport: undecodable)
        let secondStore = QuerySessionStore()
        secondStore.updateInput("alpha")

        secondStore.start(lease: try XCTUnwrap(second.beginQueryRead()))
        await secondStore.awaitRunCompletion()

        XCTAssertEqual(secondStore.phase, .completed)
        XCTAssertEqual(secondStore.rows[0].cell(.interpretation), .count(1))
        XCTAssertEqual(
            secondStore.rows[0].cell(.note),
            .unavailable(.responseNotSafelyReadable)
        )
        XCTAssertTrue(second.isConnected)
    }

    // MARK: - C-02: a scene interruption during a Query run

    /// Timeline A. The scene delivers exactly one `.active`, and it arrives
    /// while Query still owns the lane. The app must end up connected anyway.
    func testAnInterruptionDuringAQueryRunLeavesTheAppConnectedWhenItReturns() async throws {
        let stepped = SteppedHTTPTransport([])
        let model = makeConnectedModel(transport: stepped)
        let store = QuerySessionStore()
        store.updateInput("alpha")
        let identityBefore = model.accountIdentity

        store.start(lease: try XCTUnwrap(model.beginQueryRead()))
        await stepped.waitUntilParked()
        XCTAssertEqual(model.activeProviderOperation, .query)

        // Away …
        model.enterBackground()
        XCTAssertTrue(
            model.isConnected,
            "the teardown must be postponed while Query owns the lane, not applied"
        )

        // … and back, while the run is still holding the lane. This is the only
        // `.active` transition this test ever injects.
        await model.enterForeground()
        XCTAssertTrue(model.isConnected)
        XCTAssertEqual(model.activeProviderOperation, .query)

        // The abandoned read finally answers and the run unwinds.
        await stepped.release(.failure(.transport))
        await store.awaitRunCompletion()

        // The app is active, connected, and genuinely usable again — with no
        // second foreground event and no identity change.
        XCTAssertTrue(model.isConnected)
        XCTAssertNil(model.activeProviderOperation)
        XCTAssertEqual(model.accountIdentity, identityBefore)
        let next = try XCTUnwrap(model.beginQueryRead())
        next.finish()
        // Query's own truth is untouched by the interruption.
        XCTAssertEqual(store.rows[0].cell(.interpretation), .unread)
    }

    /// Timeline B. The run finishes while the app is still away, so the
    /// postponed teardown settles there and the next `.active` restores.
    func testAQueryRunFinishingWhileAwaySettlesTheOwedTeardown() async throws {
        let stepped = SteppedHTTPTransport([])
        let model = makeConnectedModel(transport: stepped)
        let store = QuerySessionStore()
        store.updateInput("alpha")
        let identityBefore = model.accountIdentity

        store.start(lease: try XCTUnwrap(model.beginQueryRead()))
        await stepped.waitUntilParked()

        model.enterBackground()
        XCTAssertTrue(model.isConnected)

        await stepped.release(.failure(.transport))
        await store.awaitRunCompletion()

        // The transient credential is not kept alive by a transition that never
        // came back: releasing the lane settles what the scene change asked for.
        XCTAssertFalse(model.isConnected)
        XCTAssertNil(model.activeProviderOperation)

        // And the ordinary foreground restore then works normally.
        await model.enterForeground()
        XCTAssertTrue(model.isConnected)
        XCTAssertEqual(
            model.accountIdentity, identityBefore,
            "restoring the same saved Token is not an account change"
        )
    }

    /// Timeline C. No scene transition at all: nothing about lifecycle changes.
    func testAQueryRunWithNoSceneTransitionChangesNoLifecycleState() async throws {
        let transport = FakeHTTPTransport([
            resolvedQueryResponse(["alpha"]),
            interpretationsResponse([]),
            phrasesResponse([]),
            notesResponse([]),
        ])
        let model = makeConnectedModel(transport: transport)
        let store = QuerySessionStore()
        store.updateInput("alpha")
        let identityBefore = model.accountIdentity

        store.start(lease: try XCTUnwrap(model.beginQueryRead()))
        await store.awaitRunCompletion()

        XCTAssertEqual(store.phase, .completed)
        XCTAssertTrue(model.isConnected)
        XCTAssertNil(model.activeProviderOperation)
        XCTAssertEqual(model.accountIdentity, identityBefore)

        // Nothing was owed, so a later background still tears down immediately.
        model.enterBackground()
        XCTAssertFalse(model.isConnected)
    }

    /// Timeline D's anchor: with no lane owner, a scene change still drops the
    /// transient credential at the moment of the transition, exactly as before.
    func testABackgroundTransitionWithNoLaneOwnerStillTearsDownImmediately() {
        let model = makeConnectedModel(transport: FakeHTTPTransport([]))
        XCTAssertTrue(model.isConnected)
        XCTAssertNil(model.activeProviderOperation)

        model.enterBackground()

        XCTAssertFalse(model.isConnected)
    }

    // MARK: - C-03: stopping is not finishing

    /// `stop()` supersedes the run, but the old run keeps the provider lane
    /// until it has actually unwound. Awaiting it is therefore a real signal,
    /// not an optional task that has already been cleared.
    func testAStoppedRunHoldsTheProviderLaneUntilItActuallyFinishes() async throws {
        let stepped = SteppedHTTPTransport([])
        let model = makeConnectedModel(transport: stepped)
        let store = QuerySessionStore()
        store.updateInput("alpha")

        store.start(lease: try XCTUnwrap(model.beginQueryRead()))
        await stepped.waitUntilParked()
        XCTAssertEqual(model.activeProviderOperation, .query)

        store.stop()

        // Stopped, but not finished: the request is still unanswered, so no
        // other provider operation may start yet.
        XCTAssertEqual(model.activeProviderOperation, .query)
        XCTAssertNil(model.beginQueryRead())
        XCTAssertNil(model.executeConfirmedWholePlan())
        let reconnected = await model.connect(token: fakeToken)
        XCTAssertFalse(reconnected, "a credential operation started over the old run")

        // Now let the abandoned resolver answer, and wait for the real thing.
        await stepped.release(resolvedQueryResponse(["alpha"]))
        await store.awaitRunCompletion()

        // The late response could not mutate the stopped result …
        XCTAssertNil(store.rows[0].vocabularyID)
        XCTAssertNil(store.rows[0].rowInability)
        XCTAssertEqual(store.rows[0].cell(.interpretation), .unread)
        XCTAssertEqual(store.phase, .stopped(.userStopped))
        // … no follow-up read was dispatched over the stopped run …
        let reads = await stepped.readCount
        XCTAssertEqual(reads, 1)
        // … and only now is the lane free.
        XCTAssertNil(model.activeProviderOperation)
        let next = try XCTUnwrap(model.beginQueryRead())
        next.finish()
    }

    // MARK: - Helpers

    private func makeConnectedModel(transport: HTTPTransport) -> CompanionViewModel {
        let model = CompanionViewModel(phraseSafetyJournal: makeTestPhraseJournal(),
            tokenStore: FakeTokenStore(),
            historyStore: InMemoryHistoryStore(),
            transportFactory: { transport },
            credentialValidationTransportFactory: {
                successfulCredentialValidationTransport()
            },
            sleeperFactory: { RecordingSleeper() },
            backgroundAssertionFactory: { FakeBackgroundExecutionAssertion() },
            preferenceDefaults: isolatedPreferenceDefaults()
        )
        var token = fakeToken
        model.installVerifiedCredentialForTesting(token: &token)
        return model
    }
}
